...
SetupLocation = "\\10.10.0.78\scratch\setup\fusioninventory"
...
'
SetupOptions = "/acceptlicense /debug=2 /runnow /server='https://myhost.my-domain.com/glpi/plugins/fusioninventory/' /S"
...
Function IsInstallationNeeded(strSetupVersion, strSetupArchitecture, strSystemArchitecture)
   ...
   objLog.WriteLine strTimeStamp & " -- Installation needed: " & strCurrentSetupVersion & " -> " & strSetupVersion
               IsInstallationNeeded = True
            End If
            Exit Function
         Else
            ...
            Err.Clear
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- Installation needed: " & strSetupVersion
            IsInstallationNeeded = True
         End If
      End If
   Else
...
Function IsSelectedForce()
   ...
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- Installation forced: " & SetupVersion
...
Function ShowMessage(strMessage)
   ...
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- " & strMessage
   End If
End Function
...
Dim nMinutesToAdvance, strCmd, strSystemArchitecture, strTempDir, WshShell, ObjFSO, objLog, strTimeStamp
Set WshShell = WScript.CreateObject("WScript.shell")

Set ObjFSO = CreateObject("Scripting.FileSystemObject")
Set objLog = objFSO.CreateTextFile("c:\Fusion_Inventory_Agent_deploy_log.txt")

strTimeStamp = GetTimeStamp ()
objLog.WriteLine strTimeStamp & " -- Beginning MAIN"
...
If (strSystemArchitecture <> "x86") And (strSystemArchitecture <> "x64") Then
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- The system architecture is unknown or not supported."
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- Deployment aborted!"
   WScript.Quit 1
Else
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- System architecture detected: " & strSystemArchitecture
End If
...
Select Case LCase(SetupArchitecture)
   Case "x86"
      ...
      strTimeStamp = GetTimeStamp ()
      objLog.WriteLine strTimeStamp & " -- Setup architecture: " & SetupArchitecture
   Case "x64"
   ...
   Case "auto"
      ...
      strTimeStamp = GetTimeStamp ()
      objLog.WriteLine strTimeStamp & " -- Setup architecture detected: " & SetupArchitecture
   Case Else
      ' The setup architecture is not supported
      strTimeStamp = GetTimeStamp ()
      objLog.WriteLine strTimeStamp & " -- The setup architecture '" & SetupArchitecture & "' is not supported."
      WScript.Quit 2
End Select
...
If (strSystemArchitecture = "x86") And (SetupArchitecture = "x64") Then
   ...
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- It isn't possible to execute a 64-bit setup on a 32-bit operative system."
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- Deployment aborted!"
   WScript.Quit 3
End If
...
If IsSelectedForce() Or IsInstallationNeeded(SetupVersion, SetupArchitecture, strSystemArchitecture) Then
   If isHttp(SetupLocation) Then
      strTimeStamp = GetTimeStamp ()
      objLog.WriteLine strTimeStamp & " -- Downloading: " & SetupLocation & "/" & Setup
      If SaveWebBinary(SetupLocation, Setup) Then
         strCmd = WshShell.ExpandEnvironmentStrings("%ComSpec%")
         strTempDir = WshShell.ExpandEnvironmentStrings("%TEMP%")
         strTimeStamp = GetTimeStamp ()
         objLog.WriteLine strTimeStamp & " -- Running: """ & strTempDir & "\" & Setup & """ " & SetupOptions
         WshShell.Run """" & strTempDir & "\" & Setup & """ " & SetupOptions, 0, True
         strTimeStamp = GetTimeStamp ()
         objLog.WriteLine strTimeStamp & " -- Scheduling: DEL /Q /F """ & strTempDir & "\" & Setup & """"
      ...
      Else
        strTimeStamp = GetTimeStamp ()
        objLog.WriteLine strTimeStamp & " -- Error downloading '" & SetupLocation & "\" & Setup & "'!"
      End If
   Else
      strTimeStamp = GetTimeStamp ()
      objLog.WriteLine strTimeStamp & " -- Running: """ & SetupLocation & "\" & Setup & """ " & SetupOptions
      'Option Explicit
...
Else
   strTimeStamp = GetTimeStamp ()
   objLog.WriteLine strTimeStamp & " -- It isn't needed the installation of '" & Setup & "'."
End If
objLog.Close
